...
def basket_add(request, product_id):
    ...
    if form.is_valid():
        basket.add(product=product, quantity=form.cleaned_data['quantity'])
    else:
...
def basket_remove(request, product_id):
    ...
    messages.success(request, f'Товар {product.name} был удалён из корзины')
    return redirect('basket')
...
def basket_buy(request):
    ...
    order = Order.objects.create(customer=customer, price=basket.get_total_price())
    ...
    message += f'</ul><p>Всего: {order.price} руб. ({len(basket)} шт.)</p>'

    basket.clear()
    messages.success(request, 'Заказ был успешно оформлен')
    send_mail(
